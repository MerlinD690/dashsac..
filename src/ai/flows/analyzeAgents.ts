...
2.  Identificar a atendente mais produtiva do dia (maior número de clientes atendidos). Esta será o "Atendente em Destaque" do dia.
3.  Identificar a atendente menos produtiva do dia (menor número de clientes atendidos).
...
1.  Analise os 'historicalData' em conjunto com os dados do dia para identificar tendências de longo prazo.
2.  **Destaque a consistência:** Qual atendente se mantém como a mais produtiva na maioria dos dias? Existe alguma atendente que consistentemente fica entre as menos produtivas? Use os números para justificar (ex: "Beatriz foi a mais produtiva em 3 dos últimos 5 dias.").
3.  **Identifique sobrecarga:** Com base no histórico, existe alguma atendente que consistentemente atende um volume de clientes muito maior que as outras? Sinalize isso como um ponto de atenção para possível sobrecarga.
4.  Compare a produtividade geral: O total de clientes de hoje foi maior ou menor que a média dos últimos dias?
5.  Gere insights e recomendações ESTRATÉGICAS. Por exemplo, se uma atendente está consistentemente sobrecarregada, sugira uma redistribuição de tarefas ou um bônus. Se a equipe toda está caindo de produção, sugira investigar a causa.
6.  Seja direto, use os dados e os números para embasar cada afirmação na análise histórica.
